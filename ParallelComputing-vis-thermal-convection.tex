...
in a scripted animation loop. Table \ref{tab:parallelgpu-tab} summarizes our results,
with runs distributed among 4, 8 and 12 compute nodes.
...
\begin{table}[htb]
  ...
  \begin{tabular}{lccc}
    ...
    \hline
     4-node GPU RayCaster & 8.2 secs &  5.26 \\
     8-node GPU RayCaster & 4.6 secs &  3.70 \\
    12-node GPU RayCaster & 2.9 secs &  2.27 \\
    \hline
     4-node NVIDIA IndeX & 18.9 secs &  1.78\\
     8-node NVIDIA IndeX & 12.4 secs &  0.67\\
    12-node NVIDIA IndeX & 9.8 secs &  0.55\\
    \hline
    \hline
     4-node Intel OSPRay & 1.35 secs &  0.70\\
     8-node Intel OSPRay & 1.18 secs &  0.48\\
    12-node Intel OSPRay & 1.14 secs &  0.37\\
    \hline
...
For the GPU-based methods, less data is transferred over from CPU memory to GPU memory.
...
Likewise for the CPU-based implementation, where the build-up of the ray-tracing
acceleration structures is directly proportional to the size of the grid.
...
the NVIDIA IndeX has the most significant bottleneck. Discussions with NVIDIA
...
of the overal scene and parallel frame compositing. 
