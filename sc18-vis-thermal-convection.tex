...
%Fig. \ref{fig:overview}, the amount of imposed shear is a key factor
...
Tesla P100 GPU (16 Gb RAM), so priority is to evaluate the GPU-based implementations.
ParaView "Ray Cast Only" mode is also available but we have found its performance far
...
16Gb NVIDIA's Pascal GPUs. For the size of our simulations outputs, we are actually forced to use data-parallel
...
after the time ParaView has collected all the data and created a bounding-box representation. This startup cost for the first image is also of paramount importance in a movie-making scenario, where data are read from disk, a single image is computed, and the whole visualization pipeline and hardware resources are flushed to get the next timestep.
\newline
...
To further evaluate the memory cost on a single node, we restricted our test sample to a quarter-size domain of the original grid, i.e. 2.28G voxels ($ 1730 \times 3455 \times 384 $).

\begin{table}[htb]
  \centering
  \caption{
    Initialization time for the first image and memory costs on one node.
  }
...
increasing the run-time cost of the visualization. Our data domain is quite large and we are not able to load a half-size domain on a single node equipped
with GPUs. Indeed, the 64Gb RAM on the node is the hard limitation. On the other hand,
the nodes on the multi-core partition of Piz daint with dual-Xeon chips and 128Gb of RAM can accomodate the data, in quarter-size,
half-size, or even full-size domain. We were able to measure the startup cost for
the first image. We are pleased to report that the OSPRay based implementation,
using a single compute node and 72 execution threads has linear costs for
the startup image. We have tested the quarter-size, half-size and the full domain
and we report the delivery of the first image in 2.1, 4.0, and 8.0 seconds, respectively. the associated cost in RAM is also linear, at 27.6Gb, 55.1Gb and 110 Gb, respectively.
...
\subsection{Parallel rendering costs}
